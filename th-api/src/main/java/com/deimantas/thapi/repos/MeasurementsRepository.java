...
public interface MeasurementsRepository extends JpaRepository<MeasurementsEntity, Long> {
	...
	@Query(value = "SELECT * FROM measurements_entity WHERE sensor_id = :sId AND time BETWEEN current_date - :sDay AND current_date - (:sDay - 1) ORDER BY time ASC", nativeQuery = true)
